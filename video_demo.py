...
def draw_keypoints(img, all_keypoints_by_type, delta_x = 1, delta_y = 1):
    working_img = img.copy()
    for index, entry_id in enumerate(all_keypoints_by_type):
        if index != 6:
            for keypoint in entry_id:
                x, y, heatmap_value, _ = keypoint

                x = int(x * delta_x)
                y = int(y * delta_y)
                # if heatmap_value > 0.3:
                cv2.putText(working_img, str(index), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.circle(working_img, (x, y), 3, (255, 0, 255), -1)
            # plt.imshow(working_img)
            # plt.show()
    return working_img

def draw_poses(img, pose_entries, num_keypoints, all_keypoints, delta_x = 1, delta_y = 1):
    current_poses = []
...
def run_demo_video(net, frame_provider, device):
    ...
    for frame_id, image in enumerate(frame_provider):
        ...
        #     model_image_input = model_image_input.cuda().to(device)
        ...
        # img_pose = draw_poses(image_original, pose_entries, num_keypoints, all_keypoints, delta_x, delta_y)
        ...
        # img_kp = draw_keypoints(image_original, all_keypoints_by_type, delta_x, delta_y)
