...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
    ...
    # Number of_heatmaps = number of keypoints to detect + 1 for a background
    # Number of PAF = number of pairs grouping * 2 
    net = PoseEstimationWithMobileNet(num_refinement_stages, num_heatmaps=7, num_pafs=10)
    ...
    for epochId in range(current_epoch, 280):
        ...
        for batch_data in train_loader:
            ...
            # plt.imshow(np.transpose(original_images[0], (1, 2, 0)))
            # plt.show()
            ...
            gauss_images = torch.FloatTensor(gauss_images)

            # gauss_images = gauss_images.cpu().numpy()
            # plt.imshow(np.transpose(gauss_images[0], (1, 2, 0)))
            # plt.show()

            gauss_images = gauss_images.cuda()
            #####
            """
            ...
            # stages_output = net(gauss_images)
            ...
            if num_iter % log_after == 0:
                ...
                for loss_idx in range(len(total_losses) // 2):
                    ...
                    writer.add_scalar('paf_loss_stage' + str(loss_idx + 1), total_losses[loss_idx * 2 + 1] / log_after, num_iter)
                    writer.add_scalar('heatmap_loss_stage' + str(loss_idx + 1), total_losses[loss_idx * 2] / log_after, num_iter)
            ...
            if num_iter % val_after == 0:
                print('time elapsed epoch {}: {:.3f}'.format(epochId, time.time() - start_epoch))
                print('time elapsed begining: {:.3f}'.format(time.time() - start))
                # print('Validation...')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val-after', type=int, default=10,
