...
from modules.keypoints import BODY_PARTS_KPT_PIG_IDS, BODY_PARTS_PAF_PIG_IDS
...
class Pose:
    ...
    kpt_names = ['nose', 'neck', 'l_ear', 'r_ear', 'back', 'tail']
    num_kpts = len(kpt_names)
    sigmas = np.array([.26, .79, .35, .35, 1.07, .89],
                       dtype=np.float32) / 10.0
    ...
    def draw(self, img, scale_x=1, scale_y=1):
        ...
        for part_id in range(len(BODY_PARTS_PAF_PIG_IDS) - 2):
...
# Modified to compute distance within only one keypoint 
def get_similarity(a, b, threshold=0.5):
    ...
    if a.keypoints[kpt_id, 0] != -1 and b.keypoints[kpt_id, 0] != -1:
        distance = np.sqrt( (a.keypoints[kpt_id, 0] - b.keypoints[kpt_id, 0]) ** 2 + (a.keypoints[kpt_id, 1] - b.keypoints[kpt_id, 1]) ** 2)
...
def track_poses(previous_poses, current_poses, threshold=3, smooth=False):
    ...
    for current_pose in current_poses:
        ...
        best_matched_iou = 10000000000000
